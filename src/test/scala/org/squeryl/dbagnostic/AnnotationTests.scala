...
class AnnotationTests extends AnyFunSuite with Matchers {
  ...
  def reflectionBrokenIn_2_9_0_RCx = {
    ...
    val _isPersistedFmd =
      descendantOfKeyedObjects.posoMetaData.findFieldMetaDataForProperty(
        "_isPersisted"
      )

    if (_isPersistedFmd.isDefined)
      fail(
        "testMetaData" + " failed, @transient annotation of field _isPersisted was not effective."
      )
    ...
    assert(
      brandNameMD.columnName == "BRAND_NAME",
      "expected 'BRAND_NAME' got " + brandNameMD.columnName
    )
    ...
    assert(
      yearOfManufacture.length == 4,
      "expected 4 got " + yearOfManufacture.length
    )

    val zozo = toasters.findFieldMetaDataForProperty("zozo").get
    assert(
      zozo.columnName == "Zozo12",
      "expected 'zozo' got " + zozo.columnName
    )
...
//    assert(dateOfPurchase.length == -1, "expected -1 got " + dateOfPurchase.length)
  ...
  /** There has been a Scala bug with obtaining a Class[_] member in
    * annotations, if this test fails, it means that Scala has regressed TODO:
    * file a bug
    */
  test("scalaReflectionTests") {
    ...
    assert(
      classOf[Int].isAssignableFrom(t1),
      "expected classOf[Int], got " + t1.getName
    )
    assert(
      classOf[Long].isAssignableFrom(t2),
      "expected classOf[Long], got " + t2.getName
    )
    assert(
      classOf[String].isAssignableFrom(t3),
