...
class Playlist(val name: String, val path: String) extends MusicDbObject {
  ...
  def songsOf(artistId: Long) =
    from(playlistElements, songs)((ple, s) =>
      where(
        id === ple.playlistId and ple.songId === s.id and s.artistId === artistId
      )
        select s
    )
}
...
class TestData {
  ...
  val besameMama = mongoSantaMaria.newSong(
    "Besame Mama",
  ...
  val funkAndLatinJazz = playlists.insert(
    new Playlist("Funk and Latin Jazz", "c:/myPlayLists/funkAndLatinJazz")
  )
}

abstract class KickTheTires
  ...
  test("kick tires") {
    ...
    val decadeOf1960 =
      playlists.insert(new Playlist("1960s", "c:/myPlayLists/funkAndLatinJazz"))
    ...
    val songsFromThe60sInFunkAndLatinJazzPlaylist =
      from(songs)(s =>
        where(
          s.id in from(funkAndLatinJazz.songsInPlaylistOrder)(s2 =>
            select(s2.id)
          )
        )
          select s
    ...
    assert(
      c == 1,
      "expected 1, got " + c + "playList.id:" + funkAndLatinJazz.id + ", artist.id:" + herbyHancock.id
    )
