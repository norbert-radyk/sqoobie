...
class TestData(schema: MusicDb) {
  ...
  val freedomSound = songs.insert(
    new Song(
      ...
      ponchoSanchez.id,
      ponchoSanchez.id,
      freedomSoundAlbum.id,
      Jazz,
      Some(Latin)
    )
  )

  val expectedSongCountPerAlbum =
    List((congaBlue.title, 2), (freedomSoundAlbum.title, 1))
}

abstract class MusicDbTestRun
  ...
  test("OuterJoinWithSubQuery") {
    ...
    artistIdsWithSongs shouldBe Set(
      herbyHancock.id,
      ponchoSanchez.id,
      mongoSantaMaria.id
    )
  ...
  test("OuterJoinInOuter", SingleTestRun) {
    ...
    val _ =
      join(artists, j2.leftOuter)((a, j2_) =>
        select(a, j2)
...
//    testKeyedEntityImplicitLookup
    ...
    def basicSelectUsingWhereOnQueryableNested =
      basicSelectUsingWhereOnQueryable.where(a =>
        a.id === testInstance.mongoSantaMaria.id
      )
    ...
    val ponchoSongs =
      List(besameMama.title, freedomSound.title, watermelonMan.title)

    validateQuery(
      "songsFeaturingPoncho",
      songsFeaturingPoncho,
      (s: Song) => s.title,
      ponchoSongs
    )
    ...
    def selfJoinNested3Level =
      from(
        from(
          from(artists)(a =>
            where(a.id === testInstance.ponchoSanchez.id) select a
          )
        )(a => select(a))
      )(a => select(a))
  ...
  test("exerciseASTRenderingOfExportSelectElements") {
    val nestedCountCDs =
      from(from(countCds(cds))(m => select(m)))(m => select(m))
  ...
  test("TimestampDownToMillis") {
    ...
    if (
    ...
    ) { // FIXME or investigate millisecond handling of each DB:

      var mongo =
        artists.where(_.firstName === mongoSantaMaria.firstName).single
      // round to 0 second :
      mongo = _truncateTimestampInTimeOfLastUpdate(mongo)
      ...
      artists.update(mongo)
      val shouldBeSome = artists
        .where(a =>
          a.firstName === mongoSantaMaria.firstName and a.timeOfLastUpdate === tX2
        )
        .headOption

      if (shouldBeSome.isEmpty)
        org.squeryl.internals.Utils
          .throwError("testTimestampDownToMillis" + " failed.")
  ...
  test("nvl followed by >= on Timestamp") {
    ...
    from(artists)(a =>
      select(nvl(a.created, new Timestamp(10L)) >= timestamp)
    ).size should be >= 0
  }
  ...
  test("validateScalarQuery1") {
    ...
    assert(
      cdCount == 2,
      "exprected 2, got " + cdCount + " from " + countCds2(cds)
    )
  ...
  test("Update1") {
    ...
    assert(
      ac.lastName == "Karon",
      "testUpdate1" + " failed, expected Karon, got " + ac.lastName
    )
  }
  ...
  test("KeyedEntityImplicitLookup") {
    ...
    assert(
      ac.id == alainCaron.id,
      "expected " + alainCaron.id + " got " + ac.id
    )
  }
  ...
  test("DeleteVariations") {
    ...
    assert(
      artists.delete(artistForDelete.id),
      "delete returned false, expected true"
    )

    assert(
      artists.lookup(artistForDelete.id).isEmpty,
      "object still exist after delete"
    )
  ...
  test("DynamicQuery1") {
    ...
    assert(
      allSongs == songsInhibited,
      "query inhibition failed, expected " + allSongs + ", got " + songsInhibited
    )
  ...
  test("DynamicQuery2") {
    ...
    assert(
      t._1.isEmpty,
      "inhibited table in query should have returned None, returned " + t._1
    )
...
//  }
  ...
  test("Enums IN") {
    ...
    mainstream.size shouldBe songs
      .where(s => s.genre === gs(0) or s.genre === gs(1))
      .size
  }
...
//    println(cds.where(_.title in q2).statement)
  ...
  test("Inhibit both sides of LogicalBoolean") {
    from(artists)(a =>
      where(
        (a.age === 1000000).inhibitWhen(true) and (a.id between (999, 1000))
          .inhibitWhen(true)
      )
        select a
    ).toList.size should be > (0)
