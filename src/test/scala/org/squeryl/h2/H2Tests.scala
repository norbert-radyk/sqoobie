...
trait H2_Connection extends DBConnector with H2_ConnectionCommon {
  def sessionCreator(): Option[() => AbstractSession] = connectToDbCommon(
    Session.create(_, new H2Adapter)
  )
}
...
class H2_NestedLeftOuterJoinTest
    extends NestedLeftOuterJoinTest
    with H2_Connection
class H2_SchoolDbMutableRelations
...
class H2_LogicalBooleanObjTests
    extends LogicalBooleanObjTests
    with H2_Connection

class H2_CommonTableExpressions
...
class H2_LazyNestedLeftOuterJoinTest
    extends NestedLeftOuterJoinTest
    with H2_LazyConnection
class H2_LazySchoolDbMutableRelations
    extends mutablerelations.SchoolDb2MetableRelations
    with H2_LazyConnection
...
class H2_LazyTestCustomTypesMode
    extends customtypes.TestCustomTypesMode
    with H2_LazyConnection
...
class H2_LazyConnectionClosing
    extends ConnectionClosingTest
    with H2_LazyConnection {
  def dbSpecificSelectNow: String = "select now()"
}
class H2_LazyLogicalBooleanObjTests
    extends LogicalBooleanObjTests
    with H2_LazyConnection
