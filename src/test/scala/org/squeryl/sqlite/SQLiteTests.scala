...
trait SQLite_ConnectionCommon extends DBConnector {
  def connectToDbCommon(
      sessionFunc: Connection => AbstractSession
  ): Option[() => AbstractSession] = {
    if (config.hasProps("sqlite.connectionString")) {
      Class.forName("org.sqlite.JDBC")
      Some(() => {
        val c = java.sql.DriverManager.getConnection(
          config.getProp("sqlite.connectionString")
        )
        val s = sessionFunc(c)
        s
      })
...
trait SQLite_Connection extends DBConnector with SQLite_ConnectionCommon {
  def sessionCreator(): Option[() => AbstractSession] = connectToDbCommon(
    Session.create(_, new SQLiteAdapter)
  )
}
...
class SQLite_NestedLeftOuterJoinTest
    extends NestedLeftOuterJoinTest
    with SQLite_Connection
class SQLite_SchoolDbMutableRelations
...
class SQLite_TestCustomTypesMode
    extends customtypes.TestCustomTypesMode
    with SQLite_Connection
...
class SQLite_LazyNestedLeftOuterJoinTest
    extends NestedLeftOuterJoinTest
    with SQLite_LazyConnection
class SQLite_LazySchoolDbMutableRelations
    extends mutablerelations.SchoolDb2MetableRelations
    with SQLite_LazyConnection
class SQLite_LazyTransactionTests
    extends TransactionTests
    with SQLite_LazyConnection
class SQLite_LazySchoolDb2
    extends schooldb2.SchoolDb2Tests
    with SQLite_LazyConnection
class SQLite_LazySchoolDb
...
class SQLite_LazyTestCustomTypesMode
    extends customtypes.TestCustomTypesMode
    with SQLite_LazyConnection
class SQLite_LazyKickTheTires
...
class SQLite_LazyConnectionClosing
    extends ConnectionClosingTest
    with SQLite_LazyConnection {
  def dbSpecificSelectNow: String = "select CURRENT_TIMESTAMP"
}
class SQLite_LazyLogicalBooleanObjTests
    extends LogicalBooleanObjTests
    with SQLite_LazyConnection
