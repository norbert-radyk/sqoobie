...
trait Derby_Connection extends DBConnector {
  def sessionCreator(): Option[() => Session] = {
    if (
      config.hasProps("derby.connectionString", "derby.user", "derby.password")
    ) {
...
class Derby_NestedLeftOuterJoinTest
    extends NestedLeftOuterJoinTest
    with Derby_Connection
class Derby_SchoolDbMutableRelations
...
class Derby_LogicalBooleanObjTests
