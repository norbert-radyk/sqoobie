...
object StatementHasher {
  ...
  private case class StatementCaseClass4HashGeneration(
      sql: String,
      definitionOrCallSite: String
  )
...
class Statement(
    ...
    val hash: Int,
    var statementHashCollisionNumber: Int
  ...
  def this(sql: String, definitionOrCallSite: String) = {
    ...
    this(
      sql,
      definitionOrCallSite,
      StatementHasher.hash(sql, definitionOrCallSite),
      0
    )
  }
...
object Measure extends Enumeration {
  type Measure = Value
  val AvgExecTime, InvocationCount, CumulativeExecutionTime, AvgResultSetSize =
    Value
}
...
object StatsSchema extends Schema {
  ...
  def invocationStats =
    from(statementInvocations)((si) =>
      groupBy(si.statementHash, si.statementHashCollisionNumber)
        compute (avg(si.executeTime), count, sum(si.executeTime), nvl(
          avg(si.rowCount),
          0
        ))
    )
  ...
  def topRankingStatements(topN: Int, measure: Measure) =
    from(invocationStats, statements)((si, s) =>
      where(
        si.key._1 === s.hash and si.key._2 === s.statementHashCollisionNumber
      )
        select (new StatLine(
          s,
          si.measures._1.get,
  ...
  def recordEndOfIteration(
    ...
    update(statementInvocations)(si =>
      where(si.id === statementInvocationId)
        set (si.iterationEndTime := Some(iterationEndTime), si.rowCount := Some(
          rowCount
        ))
    )
  }

  private def _lookupOrCreateStatementAndReturnKey(
      se: StatementInvocationEvent
  ) = {
