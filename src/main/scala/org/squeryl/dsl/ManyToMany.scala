...
trait ManyToManyRelation[L, R, A] extends Relation[L, R] {
  ...
  def rightStateful(rightSide: R) =
    new StatefulManyToMany[L, A](right(rightSide))
}

/** This trait is what is referred by both the left and right side of a
  * manyToMany relation. Type parameters are : O: the type at the "other" side
  * of the relation A: the association type i.e. the entity in the "middle" of
  * the relation
  *
  ...
  * relation, but in a given relation
  */
trait ManyToMany[O, A] extends Query[O] {
  ...
  /** @param a:
    ...
    *
    * Sets the foreign keys of the association object to the primary keys of the
    * left and right side, this method does not update the database, changes to
    * the association object must be done for the operation to be persisted.
    * Alternatively the method 'associate(o, a)' will call this assign(o, a) and
    * persist the changes.
    *
    * @return
  ...
  /** @param a:
    *   the association object
    *
    * Calls assign(o,a) and persists the changes the database, by inserting or
    * updating 'a', depending on if it has been persisted or not.
    *
    * @return
    *   the 'a' parameter is returned
    */
  ...
  /** Creates a new association object 'a' and calls associate(o,a)
    *
    * Note that this method will fail if the association object has NOT NULL
    * constraint fields apart from the foreign keys in the relations
    */
  def associate(o: O): A

  /** Causes the deletion of the 'Association object' between this side and the
    * other side of the relation.
    * @return
    *   true if 'o' was associated (if an association object existed between
    *   'this' and 'o') false otherwise
    */
  ...
  /** @return
    *   a Query of Tuple2 containing all objects on the 'other side' along with
    *   their association object
    */
...
class StatefulManyToMany[O, A](val relation: ManyToMany[O, A])
  ...
  def dissociate(o: O): Boolean = {
    ...
    assert(
      b1 == b2,
      "'MutableManyToMany out of sync " + o
        .asInstanceOf[AnyRef]
        .getClass
        .getName + " with id=" +
        relation.kedL
...
trait OneToMany[M] extends Query[M] {
  ...
  /** @param m
    *   the object on the 'many side' to be associated with this
    *
  ...
  /** Calls 'assign(m)' and persists the changes the database, by inserting or
    * updating 'm', depending on if it has been persisted or not.
    *
    * @return
    *   the 'm' parameter is returned
    */
