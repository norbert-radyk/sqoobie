...
trait TypedExpression[A1, T1] extends ExpressionNode {
  ...
  def div[T3 >: T1 <: TNumeric, T2 <: T3, A2, A3, A4, T4](
      ...
      tf: Floatifier[T3, A4, T4]
  ): TypedExpression[A4, T4] = tf.floatify(new BinaryOperatorNode(this, e, "/"))
  ...
  def isDistinctFrom[A2, T2](b: TypedExpression[A2, T2])(implicit
      ev: CanCompare[T1, T2]
  ) = new BinaryOperatorNodeLogicalBoolean(this, b, "IS DISTINCT FROM")
  def isNotDistinctFrom[A2, T2](b: TypedExpression[A2, T2])(implicit
      ev: CanCompare[T1, T2]
  ) = new BinaryOperatorNodeLogicalBoolean(this, b, "IS NOT DISTINCT FROM")

  def ===[A2, T2](
      q: Query[Measures[A2]]
  )(implicit tef: TypedExpressionFactory[A2, T2], ev: CanCompare[T1, T2]) =
  ...
  def gt[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ): LogicalBoolean = new BinaryOperatorNodeLogicalBoolean(
    this,
    q.copy(asRoot = false, Nil).ast,
    ">"
  )
  def gte[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ): LogicalBoolean = new BinaryOperatorNodeLogicalBoolean(
    this,
    q.copy(asRoot = false, Nil).ast,
  ...
  def lt[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ): LogicalBoolean = new BinaryOperatorNodeLogicalBoolean(
    this,
    q.copy(asRoot = false, Nil).ast,
  ...
  def lte[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ): LogicalBoolean = new BinaryOperatorNodeLogicalBoolean(
    this,
    q.copy(asRoot = false, Nil).ast,
  ...
  def >[A2, T2](q: Query[A2])(implicit cc: CanCompare[T1, T2]): LogicalBoolean =
    gt(q)
  def >=[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ): LogicalBoolean = gte(q)
  def <[A2, T2](q: Query[A2])(implicit cc: CanCompare[T1, T2]): LogicalBoolean =
    lt(q)
  def <=[A2, T2](q: Query[A2])(implicit
      cc: CanCompare[T1, T2]
  ...
  def isNotNull = new PostfixOperatorNode("is not null", this)
    with LogicalBoolean

  def between[A2, T2, A3, T3](
      b1: TypedExpression[A2, T2],
      b2: TypedExpression[A3, T3]
  )(implicit ev1: CanCompare[T1, T2], ev2: CanCompare[T2, T3]) =
    new BetweenExpression(this, b1, b2)

  def like[A2, T2 <: TOptionString](s: TypedExpression[A2, T2])(implicit
      ev: CanCompare[T1, T2]
  ) = new BinaryOperatorNodeLogicalBoolean(this, s, "like")

  def ilike[A2, T2 <: TOptionString](s: TypedExpression[A2, T2])(implicit
      ev: CanCompare[T1, T2]
  ...
  def is(columnAttributes: AttributeValidOnNumericalColumn*)(implicit
  ...
  def in[A2, T2](
      t: Iterable[A2]
  )(implicit cc: CanCompare[T1, T2]): LogicalBoolean =
    new InclusionOperator(
      this,
      new RightHandSideOfIn(new ConstantExpressionNodeList(t, mapper)).toIn
    )
  ...
  def notIn[A2, T2](
    ...
    new ExclusionOperator(
      this,
      new RightHandSideOfIn(new ConstantExpressionNodeList(t, mapper)).toNotIn
    )
  ...
  def defaultsTo[B](b: B)(implicit
      B: B => TypedExpression[A1, T1]
  ): DefaultValueAssignment =
    new DefaultValueAssignment(_fieldMetaData, b: TypedExpression[A1, T1])

  /** TODO: make safer with compiler plugin Not type safe ! a
    * TypedExpressionNode[T] might not be a SelectElementReference[_] that
    * refers to a FieldSelectElement...
    */
...
class TypedExpressionConversion[A1, T1](
    val e: ExpressionNode,
    bf: TypedExpressionFactory[A1, T1]
...
trait FloatTypedExpressionFactory[A1, T1]
    extends TypedExpressionFactory[A1, T1]
    with IdentityFloatifier[A1, T1] {
  self: JdbcMapper[_, A1] =>
  def floatify(v: ExpressionNode): TypedExpressionConversion[A1, T1] = convert(
    v
  )
}
...
trait JdbcMapper[P, A] {
  ...
  def map(rs: ResultSet, i: Int): A = convertFromJdbc(
    extractNativeJdbcValue(rs, i)
  )
}
...
abstract class NonPrimitiveJdbcMapper[P, A, T](
  ...
  def extractNativeJdbcValue(rs: ResultSet, i: Int): P =
    primitiveMapper.extractNativeJdbcValue(rs, i)
...
trait TypedExpressionFactory[A, T] {
  ...
  def createConstant(a: A) =
    new ConstantTypedExpression[A, T](
      a,
      thisAnyRefMapper.convertToJdbc(a),
      Some(this)
    )
  ...
  /** Converts the argument into a TypedExpression[A,T], the resulting
    * expression is meant to be equivalent in terms of SQL generation, the
    * conversion is only at the type level
    */
...
trait IntegralTypedExpressionFactory[A1, T1, A2, T2]
  ...
  def floatify(v: ExpressionNode): TypedExpressionConversion[A2, T2] =
    floatifyer.convert(v)
  def floatifyer: TypedExpressionFactory[A2, T2]
}

trait DeOptionizer[P1, A1, T1, A2 >: Option[A1] <: Option[A1], T2]
