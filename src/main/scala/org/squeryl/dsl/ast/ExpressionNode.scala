...
trait ExpressionNode {
  ...
  private def _visitDescendants(
    ...
    n.children.foreach(child =>
      _visitDescendants(child, Some(n), depth + 1, visitor)
    )
  }
  ...
  def cast[A, T](
      typ: String
  )(implicit tef: TypedExpressionFactory[A, T]): TypedExpression[A, T] =
    new CastExpressionNode(this, typ) with TypedExpression[A, T] {
      override def mapper = tef.createOutMapper
    }
}

class ListExpressionNode(override val children: List[ExpressionNode])
...
class RowValueConstructorNode(override val children: List[ExpressionNode])
    extends ExpressionNode {
  override def doWrite(sw: StatementWriter) = {
    // sw.write("ROW")
    sw.write("(")
...
class EqualityExpression(
    override val left: TypedExpression[_, _],
    override val right: TypedExpression[_, _]
...
trait BaseColumnAttributeAssignment {
  ...
  def isIdFieldOfKeyedEntityWithoutUniquenessConstraint =
    isIdFieldOfKeyedEntity && !(columnAttributes.exists(
      _.isInstanceOf[PrimaryKey]
    ) || columnAttributes.exists(_.isInstanceOf[Unique]))
...
class CompositeKeyAttributeAssignment(
    val group: CompositeKey,
    _columnAttributes: collection.Seq[ColumnAttribute]
  ...
  override def isIdFieldOfKeyedEntity = {
    ...
    fmdHead.parentMetaData.viewOrTable.ked
      .exists(_.idPropertyName == group._propertyName)
  }
...
class ColumnAttributeAssignment(
    val left: FieldMetaData,
    val columnAttributes: collection.Seq[ColumnAttribute]
) extends BaseColumnAttributeAssignment {
...
class ConstantExpressionNodeList[T](
  ...
  def doWrite(sw: StatementWriter) =
    if (sw.isForDisplay)
      sw.write(
        ConstantExpressionNodeList.this.value
          .map(e => "'" + e + "'")
          .mkString(",")
      )
    else {
      sw.write(
        ConstantExpressionNodeList.this.value.toSeq.map(z => "?").mkString(",")
      )
      ConstantExpressionNodeList.this.value.foreach(z =>
        sw.addParam(
          ConstantExpressionNodeListParam(
            z.asInstanceOf[AnyRef],
            ConstantExpressionNodeList.this
          )
        )
      )
    }
...
class FunctionNode(val name: String, val args: collection.Seq[ExpressionNode])
    extends ExpressionNode {
...
class PostfixOperatorNode(val token: String, val arg: ExpressionNode)
    extends ExpressionNode {
...
class PrefixOperatorNode(
  ...
  override def toString =
    "'PrefixOperatorNode:" + operatorToken + inhibitedFlagForAstDump
...
trait QueryableExpressionNode
  ...
  /** When the join syntax is used, isMemberOfJoinList is true if this instance
    * is not in the from clause but a 'join element'.
    */
  ...
  def getOrCreateSelectElement(
      fmd: FieldMetaData,
      forScope: QueryExpressionElements
  ): SelectElement

  def getOrCreateAllSelectElements(
      forScope: QueryExpressionElements
  ): Iterable[SelectElement]
...
/** Update, delete and insert statement are not built with AST nodes, (for
  * example Table[].update), although some portions of these statements (where
  * clauses are sometimes built with it. The StatisticsListener needs to view
  * every expression call as an AST, which is the reason for this class. AST are
  * meant to be "non rendered", i.e. agnostic to specific DatabaseAdapter, this
  * DummyExpressionHolder is an exception. TODO: unify expression building to be
  * completely AST based.
  */
class DummyExpressionHolder(val renderedExpression: String)
    extends ExpressionNode {
...
class RightHandSideOfIn[A](
    val ast: ExpressionNode,
    val isIn: Option[Boolean] = None
) extends ExpressionNode {
...
class UnionExpressionNode(val kind: String, val ast: ExpressionNode)
    extends ExpressionNode {
  def doWrite(sw: StatementWriter) = {
    sw.write(kind)
    sw.nextLine
...
class QueryValueExpressionNode[A1, T1](
    val ast: ExpressionNode,
    override val mapper: OutMapper[A1]
) extends TypedExpression[A1, T1] {
  def doWrite(sw: StatementWriter) = {
    ast.write(sw)
  }
