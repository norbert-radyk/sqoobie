...
import org.squeryl.dsl.internal.{
  JoinedQueryable,
  InnerJoinedQueryable,
...
trait JoinSignatures {
  ...
  implicit def queryable2JoinPrecursor[A](q: Queryable[A]): JoinPrecursor[A] =
    new JoinPrecursor[A](q)

  implicit def queryable2RightInnerJoinedQueryable[A](
      q: Queryable[A]
  ): InnerJoinedQueryable[A] = new InnerJoinedQueryable[A](q, "")

  def join[A, B1, C](q: Queryable[A], q1: JoinedQueryable[B1])(
      f: (A, B1) => JoinQueryYield1[C]
  ): Query[C] =
  ...
  )(f: (A, B1, B2) => JoinQueryYield2[C]): Query[C] =
  ...
  )(f: (A, B1, B2, B3) => JoinQueryYield3[C]): Query[C] =
    from(q, q1, q2, q3)((a: A, b1: B1, b2: B2, b3: B3) =>
      f(a, b1, b2, b3).queryYield
    )
  ...
  def join[A, B1, B2, B3, B4, C](
    ...
    from(q, q1, q2, q3, q4)((a: A, b1: B1, b2: B2, b3: B3, b4: B4) =>
      f(a, b1, b2, b3, b4).queryYield
    )
  ...
  def join[A, B1, B2, B3, B4, B5, B6, B7, B8, B9, C](
      ...
      f: (A, B1, B2, B3, B4, B5, B6, B7, B8, B9) => JoinQueryYield9[C]
  ): Query[C] =
