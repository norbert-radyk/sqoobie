...
class BaseQueryYield[G](
    val queryElementzz: QueryElements[_],
    val selectClosure: () => G
...
class GroupQueryYield[K](
  ...
  override def invokeYieldForAst(
    ...
    val offset = 1
    val (m, nodes) = _createColumnToTupleMapper(
      q,
      groupByClauseClosure(),
      offset,
      isForGroup = true
    )
...
class GroupWithMeasuresQueryYield[K, M](
  ...
  override def invokeYield(rsm: ResultSetMapper, rs: ResultSet) =
    new GroupWithMeasures(
      rsm.groupKeysMapper.get.mapToTuple(rs),
      rsm.groupMeasuresMapper.get.mapToTuple(rs)
    )
  ...
  override def invokeYieldForAst(
    ...
    val (km, knodes) = _createColumnToTupleMapper(
      q,
      _groupByClauseClosure(),
      offset,
      isForGroup = true
    )
    val (mm, mnodes) = _createColumnToTupleMapper(
      q,
      _computeClauseClosure(),
      offset + knodes.size,
      isForGroup = false
    )
