...
trait KeyedEntityDef[-A, K] extends OptionalKeyedEntityDef[A, K] {
  ...
  /** the counter field name for OCC, None to disable OCC (optimistic
    * concurrency control)
    */
...
/** For use with View[A] or Table[A], when A extends KeyedEntity[K], lookup and
  * delete by key become implicitly available Example :
  *
  * class Peanut(weight: Float) extends KeyedEntity[Long] val peanutJar =
  * Table[Peanut]
  *
...
/** ForeignKeyDeclaration are to be manipulated only during the Schema
  * definition (this is why all public methods have the implicit arg (implicit
  * ev: Schema))
  */
class ForeignKeyDeclaration(
  ...
  private[this] var _referentialActions
      : Option[(Option[ReferentialAction], Option[ReferentialAction])] = None
  ...
  /** Will cause a foreign key constraint to be created at schema creation time
    * : alter table <tableOfForeignKey> add foreign key (<foreignKey>)
    * references <tableOfPrimaryKey>(<primaryKey>)
    */
  ...
  /** Does the same as constrainReference, plus adds two ReferentialActions
    * (ex.: foreignKeyDeclaration.constrainReference(onDelete cascade, onUpdate
    * restrict))
    */
  def constrainReference(a1: ReferentialAction, a2: ReferentialAction)(implicit
      ev: Schema
  ) =
