...
class Table[T] private[squeryl] (
  ...
  def insert(t: T): T = StackMarker.lastSquerylStackFrame {
    ...
    val st =
      (
        _dbAdapter.supportsAutoIncrementInColumnDeclaration,
        posoMetaData.primaryKey
      ) match {
        case (true, a: Any) =>
          sess.connection.prepareStatement(
            sw.statement,
    ...
    try {
      ...
      if (cnt != 1)
        throw SquerylSQLException(
          "failed to insert.  Expected 1 row, got " + cnt
        )
...
//  def insert(t: Query[T]) = org.squeryl.internals.Utils.throwError("not implemented")
  ...
  def insert(e: Iterable[T]): Unit =
    _batchedUpdateOrInsert(
      e,
      t =>
        posoMetaData.fieldsMetaData.filter(fmd =>
          !fmd.isAutoIncremented && fmd.isInsertable
        ),
      isInsert = true,
      checkOCC = false
    )
  ...
  private def _batchedUpdateOrInsert(
    ...
    if (it.hasNext) {
      ...
      if (sess.isLoggingEnabled)
        sess.log(
          "Performing batched " + (if (isInsert) "insert"
                                   else "update") + " with " + sw.statement
        )
  ...
  private def _update(o: T, checkOCC: Boolean, ked: KeyedEntityDef[T, _]) = {
    ...
    if (cnt != 1) {
      if (checkOCC && posoMetaData.isOptimistic) {
        val version = posoMetaData.optimisticCounter.get.getNativeJdbcValue(
          o.asInstanceOf[AnyRef]
        )
        throw new StaleUpdateException(
          "Object " + prefixedName + "(id=" + ked.getId(
            o
          ) + ", occVersionNumber=" + version +
            ") has become stale, it cannot be updated under optimistic concurrency control"
        )
      } else
        throw SquerylSQLException(
          "failed to update.  Expected 1 row, got " + cnt
        )
    }
    ...
    def buildFmds(t: T): Iterable[FieldMetaData] = {
      val pkList = posoMetaData.primaryKey
        .getOrElse(
          org.squeryl.internals.Utils.throwError(
            "method was called with " + posoMetaData.clasz.getName + " that is not a KeyedEntity[]"
          )
        )
        .fold(
          pkMd => List(pkMd),
      ...
      List(
        posoMetaData.fieldsMetaData
          .filter(fmd =>
            !fmd.isIdFieldOfKeyedEntity && !fmd.isOptimisticCounter && fmd.isUpdatable
          )
          .toList,
        pkList,
  ...
  def deleteWhere(whereClause: T => LogicalBoolean)(implicit
      dsl: QueryDsl
  ): Int =
    delete(dsl.from(this)(t => dsl.where(whereClause(t)).select(t)))

  def delete[K](k: K)(implicit
    ...
    if (
      Session.currentSessionOption forall { ses =>
