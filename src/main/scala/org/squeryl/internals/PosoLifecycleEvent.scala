...
object PosoLifecycleEvent extends Enumeration {
  ...
  val BeforeInsert, AfterInsert, BeforeDelete, AfterDelete, BeforeUpdate,
      AfterUpdate, AfterSelect, Create = Value
}

class LifecycleEventInvoker(i: Iterable[LifecycleEvent], owner: View[_])
    extends PosoLifecycleEventListener {

  import PosoLifecycleEvent._

  private[this] val _beforeInsert =
    i.filter(_.e == BeforeInsert).map(_.callback)
  private[this] val _afterInsert = i.filter(_.e == AfterInsert).map(_.callback)
  private[this] val _beforeDelete =
    i.filter(_.e == BeforeDelete).map(_.callback)
  private[this] val _afterDelete = i.filter(_.e == AfterDelete).map(_.callback)
  private[this] val _beforeUpdate =
    i.filter(_.e == BeforeUpdate).map(_.callback)
  ...
  private[this] val _factory: AnyRef => AnyRef = {
    ...
    if (f.size > 1)
      org.squeryl.internals.Utils
        .throwError(owner.name + " has more than one factory defined.")
...
class PosoFactoryPercursorTable[A](target: View[_])
...
class LifecycleEventPercursorTable[A](
    target: View[_],
    e: PosoLifecycleEvent.Value
) extends BaseLifecycleEventPercursor {
...
class LifecycleEventPercursorClass[A](
    target: Class[_],
    schema: Schema,
    e: PosoLifecycleEvent.Value
) extends BaseLifecycleEventPercursor {
...
class LifecycleEvent(
    ...
    val e: PosoLifecycleEvent.Value,
