...
object Utils {
  ...
  /** Will attempt to evaluate a string expression and will catch any exception.
    * For use in circumstances when logging is needed (i.e. a fatal error has
    * already occurred and we need to log as much info as possible (i.e. put as
  ...
  private class DummyQuery4WhereClause[A, B](
      q: Queryable[A],
      whereClause: A => LogicalBoolean
  ) extends Query1[A, Int](
        q,
        a => {
