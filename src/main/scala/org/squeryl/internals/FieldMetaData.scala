...
class FieldMetaData(
  ...
  def sequenceName: String = {
    ...
    val ai = _columnAttributes
      .collectFirst { case a: AutoIncremented => a }
      .getOrElse(
        org.squeryl.internals.Utils.throwError(
          s"$this is not declared as autoIncremented, hence it has no sequenceName"
        )
      )
  ...
  if (
    isIdFieldOfKeyedEntity && !classOf[CompositeKey].isAssignableFrom(
      wrappedFieldType
    )
  ) {
    schema
      ...
      .foreach(ca => {
        ...
        if (
          ca.isInstanceOf[AutoIncremented] && !(wrappedFieldType
            .isAssignableFrom(classOf[java.lang.Long]) || wrappedFieldType
  ...
  private[squeryl] var _defaultValue: Option[ConstantTypedExpression[_, _]] =
    None
  ...
  /** If explicit db type case has been requested
    */
  def explicitDbTypeCast: Boolean = _columnAttributes
    ...
    try {
      val v0: AnyRef =
        if (v == null)
          ...
          canonicalEnumerationValueFor(
            v.asInstanceOf[java.lang.Integer].intValue
          )
  ...
  val factory = new FieldMetaDataFactory {
    ...
    def build(
      ...
      if (classOf[Product1[Any]].isAssignableFrom(clsOfField))
        customTypeFactory = _createCustomTypeFactory(
          fieldMapper,
          parentMetaData.clasz,
          clsOfField
        )
  ...
  def defaultFieldLength(fieldType: Class[_], fmd: FieldMetaData) = {
    if (classOf[String].isAssignableFrom(fieldType))
      ...
      classOf[java.math.BigDecimal]
        .isAssignableFrom(fieldType) || classOf[scala.math.BigDecimal]
                ...
                case oType :: Nil =>
                  if (classOf[Class[_]].isInstance(oType)) {
                    ...
                    trueTypeOption flatMap { trueType =>
                      val deduced = createDefaultValue(
                        fieldMapper,
                        member,
                        trueType,
                        None,
                      ...
                      Option(
                        deduced
                      ) // Couldn't create default for type param if null
                    }
                  } else {
