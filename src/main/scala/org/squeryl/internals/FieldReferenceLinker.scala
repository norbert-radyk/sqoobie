...
object FieldReferenceLinker {
  ...
  def pushExpressionOrCollectValue[T](e: () => TypedExpression[T, _]): T = {
    if (isYieldInspectionMode) {
      ...
      yi.addSelectElement(
        new ValueSelectElement(
          expr,
          yi.resultSetMapper,
          expr.mapper,
          yi.queryExpressionNode
        )
      )
      val r = expr.sample
      r
  ...
  private[this] val __lastAccessedFieldReference =
    new ThreadLocal[Option[SelectElement]]
  ...
  private[this] val _compositeKeyMembers =
    new ThreadLocal[Option[ArrayBuffer[SelectElement]]]

  /** _lastAccessedFieldReference is unique per thread, AST construction can be
    * nested and can interfere with one another, this method is used for
    * preserving the previous _lastAccessedFieldReference when a nested AST
    * construction takes place *and* during the construction of 'sample' POSOs,
    * because they are proxied, and can call their intercepted fields during
    * construction, calling the constructor for 'sample' POSO construction
    * without wrapping with this methor would have the effect of 'polluting' the
    * _lastAccessedFieldReference (issue 68).
    */
  ...
  private def _takeLastAccessedUntypedFieldReference
      : SelectElementReference[_, _] =
    FieldReferenceLinker.takeLastAccessedFieldReference match {
      case Some(n: SelectElement) =>
        new SelectElementReference(n, NoOpOutMapper)
      case None =>
        org.squeryl.internals.Utils.throwError(
          "Thread local does not have a last accessed field... this is a severe bug !"
        )
    }
  ...
  /** It is assumed that yield invocation for inspection will never be nested,
    * since a query is completely built (and it's yield inspection is done)
    * before it can be nested, this is unlikely to change, but documenting this
    * assumption was deemed useful, because this method would stop working
    * (without complaining) if (the assumption) was broken.
    */
  ...
  private def _populateSelectCols(
      yi: YieldInspection,
      q: QueryExpressionElements,
      sample: AnyRef
  ): Unit = {
...
//    else
  ...
  private def _findQENThatOwns(
      sample: AnyRef,
      q: QueryExpressionElements
  ): Option[QueryableExpressionNode] = {
  ...
  private class PosoPropertyAccessInterceptor(
    ...
    private def _intercept(
      ...
      val res =
        if (m.getName.equals("toString") && m.getParameterTypes.length == 0)
          "sample:" + viewExpressionNode.view.name + "[" + Integer.toHexString(
      ...
      if (isComposite) {
        ...
        ck._members = Some(
          _compositeKeyMembers.get.get.map(
            new SelectElementReference[Any, Any](_, NoOpOutMapper)
          )
        )
      ...
      if (fmd.isDefined) {
        ...
        if (yi != null && yi.reentranceDepth == 1)
          yi.addSelectElement(
            viewExpressionNode.getOrCreateSelectElement(
              fmd.get,
              yi.queryExpressionNode
            )
          )
        ...
        if (_compositeKeyMembers.get == null) {
          _compositeKeyMembers.remove()
          _lastAccessedFieldReference = Some(
            viewExpressionNode.getOrCreateSelectElement(fmd.get)
          )
        } else
          _compositeKeyMembers.get.get.append(
            viewExpressionNode.getOrCreateSelectElement(fmd.get)
          )
      }
