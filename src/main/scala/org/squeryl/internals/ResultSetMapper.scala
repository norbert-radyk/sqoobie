...
trait ResultSetUtils {
  ...
  def dumpRow(rs: ResultSet) = {
    ...
    (for (i <- 1 to md.getColumnCount)
      yield "#" + i + "->" + rs.getObject(i) + ":" + _simpleClassName(
...
object NoOpOutMapper extends OutMapper[Any] {
  ...
  def sample = throw new UnsupportedOperationException(
    " cannot use NoOpOutMapper"
  )

  override def typeOfExpressionToString = "NoOpOutMapper"
}

class ColumnToFieldMapper(
    val index: Int,
...
class ColumnToTupleMapper(val outMappers: Array[OutMapper[_]]) {
  ...
  def typeOfExpressionToString(idx: Int) =
    outMappers.apply(idx).typeOfExpressionToString
  ...
  def mapToTuple[T](rs: ResultSet): T = {
    ...
    val res = size match {
      ...
      case 5 =>
        (m(0).map(rs), m(1).map(rs), m(2).map(rs), m(3).map(rs), m(4).map(rs))
      case 6 =>
        (
          m(0).map(rs),
          m(1).map(rs),
      ...
      case z: Any =>
        org.squeryl.internals.Utils.throwError(
          "tuples of size " + size + " and greater are not supported"
        )
    }
...
//  }
  ...
  def isNoneInOuterJoin(rs: ResultSet): Boolean = {
    ...
    // outMappers
    for (
      col2TupleMapper <- List(groupKeysMapper, groupMeasuresMapper)
        .filter(_.isDefined)
        .map(_.get);
