...
class Schema(implicit val fieldMapper: FieldMapper) {
  ...
  /** Contains all Table[_]s in this shema, and also all
    * ManyToManyRelation[_,_,_]s (since they are also Table[_]s
    */
  ...
  private[this] val _manyToManyRelations =
    new ArrayBuffer[ManyToManyRelation[_, _, _]]
  ...
  def findAllTablesFor[A](c: Class[A]) =
    _tables
      .filter(t => c.isAssignableFrom(t.posoMetaData.clasz))
      .asInstanceOf[Iterable[Table[_]]]
  ...
  object NamingConventionTransforms {
    ...
    def snakify(name: String): String =
      THIRD_PATTERN
        .matcher(
          SECOND_PATTERN
            .matcher(FIRST_PATTERN.matcher(name).replaceAll("_$1"))
            .replaceAll("$1_$2")
        )
        .replaceAll("$1_$2")
        .toLowerCase
  ...
  /** Prints the schema to standard output, it is simply :
  ...
  private def _indexDeclarationsFor(t: Table[_]): List[String] = {
    t.posoMetaData.fieldsMetaData.flatMap { fmd =>
      _writeIndexDeclarationIfApplicable(
        fmd.columnAttributes.toSeq,
        Seq(fmd),
        None
      )
    }.toList
  ...
  private def _writeColumnGroupAttributeAssignments: collection.Seq[String] =
    for (cgaa <- _columnGroupAttributeAssignments)
      yield _writeIndexDeclarationIfApplicable(
        cgaa.columnAttributes,
        cgaa.columns,
        cgaa.name
      ).getOrElse(
        org.squeryl.internals.Utils.throwError(
          "empty attribute list should not be possible to create with DSL (Squeryl bug)."
        )
      )
  ...
  private def _dropForeignKeyConstraints = {
    ...
    for (fk <- _activeForeignKeySpecs) {
      ...
      dba.dropForeignKeyStatement(
        fk._1,
        dba.foreignKeyConstraintName(fk._1, fk._3.idWithinSchema),
        cs
      )
    }
  }
  ...
  /** returns an Iterable of (Table[_],Iterable[FieldMetaData]), the list of all
    * tables whose PK is a composite, with the columns that are part of the PK :
  ...
  /** Use this method to override the DatabaseAdapter's default column type for
    * the given field (FieldMetaData), returning None means that no override
    * will take place.
    *
    * There are two levels at which db column type can be overridden, in order
    * of precedence :
    *
    ...
    *
    * MySchema extends Schema { ... override def columnTypeFor(fieldMetaData:
    * FieldMetaData, owner: Table[_]) =
    * if(fieldMetaData.wrappedFieldType.isInstanceOf[Int) Some("number") else
    * None }
    */
  def columnTypeFor(
  ...
  class ReferentialActionImpl(token: String, ev: ReferentialEvent)
  ...
  private[squeryl] def _createForeignKeyDeclaration(
      fkColName: String,
      pkColName: String
  ) = {
  ...
  def applyDefaultForeignKeyPolicy(
      foreignKeyDeclaration: ForeignKeyDeclaration
  ) =
    foreignKeyDeclaration.constrainReference()

  /** @return
    *   a Tuple2 with (LengthOfDecimal, Scale) that will determine the storage
    *   length of the database type that map fields of type java.lang.BigDecimal
    *   Can be overridden by the Column Annotation, ex.: Column(length=22,
    *   scale=20) default is (20,16)
    */

  def defaultSizeOfBigDecimal = (20, 16)

  /** @return
    *   the default database storage (column) length for String columns for this
    *   Schema, Can be overridden by the Column Annotation ex.:
    *   Column(length=256) default is 128
    */
  ...
  protected def on[A](
      table: Table[A]
  )(declarations: A => Seq[BaseColumnAttributeAssignment]) = {
    ...
    for (ca <- colAss) ca match {
      ...
      case a: Any =>
        org.squeryl.internals.Utils
          .throwError("did not match on " + a.getClass.getName)
    }
...
//      )
  ...
  private def _addColumnGroupAttributeAssignment(
      cga: ColumnGroupAttributeAssignment
  ) =
    _columnGroupAttributeAssignments.append(cga)

  def defaultColumnAttributesForKeyedEntityId(typeOfIdField: Class[_]) =
    if (
      typeOfIdField.isAssignableFrom(classOf[java.lang.Long]) || typeOfIdField
  ...
  protected def autoIncremented(sequenceName: String) = AutoIncremented(
    Some(sequenceName)
  )
  ...
  def columns(fieldList: TypedExpression[_, _]*) = new ColGroupDeclaration(
    fieldList.map(_._fieldMetaData)
  )
  ...
  private[squeryl] lazy val _callbacks: Map[View[_], LifecycleEventInvoker] = {
    val viewEventMap: Seq[(View[_], LifecycleEvent)] =
  ...
  protected def beforeDelete[A](t: Table[A])(implicit
      ev: KeyedEntityDef[A, _]
  ) =
    new LifecycleEventPercursorTable[A](t, BeforeDelete)

  protected def beforeDelete[K, A]()(implicit
      m: ClassTag[A],
      ked: KeyedEntityDef[A, K]
  ) =
  ...
  /** Creates a ActiveRecord instance for the given the object. That allows the
    * user to save the given object using the Active Record pattern.
    *
    * @return
    *   a instance of ActiveRecord associated to the given object.
    */
  implicit def anyRef2ActiveTransaction[A](
      a: A
  )(implicit queryDsl: QueryDsl, m: ClassTag[A]) =
    new ActiveRecord(a, queryDsl, m)

  /** Active Record pattern implementation. Enables the user to insert an object
    * in its existent table with a convenient {{{save}}} method.
    */
